...
// Validar cédula (ejemplo: V-12345678 o E-12345678)
function validarCedula(cedula) {
    const regex = /^[VE]-\d{7,8}$/;
    if (!regex.test(cedula)) {
        return "La cédula debe comenzar con 'V-' o 'E-' seguido de 7 u 8 dígitos.";
    }
    return true;
}

// Validar nombre (al menos dos palabras, solo letras y espacios)
function validarNombre(nombre) {
    const palabras = nombre.trim().split(/\s+/);
    const regex = /^[A-Za-záéíóúñÁÉÍÓÚÑ\s]+$/;
    if (palabras.length < 2 || !regex.test(nombre)) {
        return "El nombre debe contener al menos dos palabras y solo letras.";
    }
    return true;
}
...
function validarEdad(edad) {
    ...
    if (isNaN(edad) || edad < 18 || edad > 100) {
        return "La edad debe ser un número entre 18 y 100 años.";
    }
    return true;
}

// Validar municipio (solo letras y no vacío)
function validarMunicipio(municipio) {
    const regex = /^[A-Za-záéíóúñÁÉÍÓÚÑ\s]+$/;
    if (!regex.test(municipio) || municipio.trim().length === 0) {
        return "El municipio debe contener solo letras y no puede estar vacío.";
    }
    return true;
}
...
function registrarParticipante() {
    ...
    const cedulaRepetida = participantes.some(part => part.cedula === cedula);
    if (cedulaRepetida) {
        alert("La cédula ingresada ya está registrada. Por favor, ingrese otra.");
        return;
    }

    // Crear el objeto participante
    const participante = { cedula, nombre, municipio, edad: Number(edad) };
